...
export const agentKeys = ['claude', 'codex', 'gemini', 'openclaw', 'qwen'] as const;
...
export const AgentDefaultOverridesSchema = z.object({
    ...
    qwen: AgentDefaultOverrideSchema.optional(),
}).passthrough().default({});
...
const codeAgentDefaults: Record<AgentKey, AgentDefaultConfig> = {
    ...
    qwen: { permissionMode: 'default', modelMode: 'default', effortLevel: null },
};

export function normalizeAgentKey(flavor: string | null | undefined): AgentKey {
    if (flavor === 'codex' || flavor === 'gemini' || flavor === 'openclaw' || flavor === 'qwen') {
        return flavor;
    }
    return 'claude';
}
